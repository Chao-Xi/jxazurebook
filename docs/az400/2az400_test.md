...
To answer, drag the appropriate tools to the correct behaviors. Each tool may be used once, more than once, or not at all. You may need to drag the split bar between panes or scroll to view content.
...
A. the Microsoft Test & Feedback extension. (**primarily used for exploratory testing of **web applications** from within a browser.**)
...
> Failures / Exceptions / Response times / Performance degradation
...
You need to create process documentation for App1. 

The solution must **include a diagram** that displays the relationships between the phases of App1 as shown in the following exhibit.
...
You have an Azure Application Insights resource named AppInsights1 that collects telemetry data generated by webapp1.
...
<mark>**You need to ensure that webapp1 sends the telemetry data at a fixed sampling rate.**</mark>
...
You create an Application Insights instance named Al1. **You need to configure App1 to use Al1.** Which file should you modify?
...
* A. the Telegraf agent. (**Not native Azure VM process monitoring**)
...
Which two values are required to configure the extension? 
...
<mark>You need to configure the Organization Settings in Azure DevOps to support Azure Pipelines for Microsoft Teams.</mark> 
...
<mark>👉 **Create a Variable Group** in the project and **link it to Key Vault**.</mark>
...
2. **Perform a custom installation of Git for Windows that includes Git Large File Storage (LFS)**
...
* Can **push to the repository** ✔️
* Meets requirement: *“actively push to repo1”*

🔹 **User2 → Triage**

* Can **manage issues and pull requests** (label, assign, close, reopen) ✔️
* Cannot push code → aligns with requirement

🔹 **User3 → Maintain**

* Can **manage the repository** (settings, branches, etc.) ✔️
* **Cannot access sensitive data** like secrets (unlike Admin) ✔️
* Meets both:
  * *“manage repo1”*
...
* **Multiple resources need same access**     → Use **user-assigned managed identity**
...
* ❌ **A. NuGet** =>  Package manager for .NET; manages packages but **does not enforce license compliance**

* ❌ **B. Maven** =>  Build tool for Java; manages dependencies but **does not check licenses automatically**

* ❌ **D. Helm** => Kubernetes package manager; unrelated to license compliance for application libraries
...
* **Why .p12?** An iOS certificate alone (usually a `.cer` file) only contains the public key. To sign an app on a build server or a distribution platform like App Center, you need both the certificate and its associated **private key**.
* In the Apple ecosystem, the standard way to export a certificate along with its private key is as a **PKCS #12** archive, which uses the **.p12** extension.
...
❌ <mark>**B. Create a webhook**. → Webhooks trigger events, they don’t copy repositories</mark>
...
<mark></mark>Increasing **code duplication** makes technical debt **worse**, not better</mark>. It:
...
* <mark>**`add` → adds the external repository as a subdirectory**</mark>
...
* <mark>You must **add a GitHub Actions workflow** that runs CodeQL analysis.</mark>
...
<mark>`https://github.com/` **[ Repository owner ]** / **[ Repository name ]** `/actions/workflows/WF1/badge.svg`></mark>
...
2.  <mark>**Grant secret permissions to kv1.** (You add an Access Policy to the Key Vault to allow the service principal from Step 1 to "Get" and "List" secrets).</mark>
...
* ❌ **B. Who should have performed better?** → Blame-oriented, ضAgileprinciples
...
You need to ensure that developers receive Slack notifications when there are pull requests created for Contoso App. What should you run in Slack? 
...
You need to assess the performance of the project. 

The solution must identify the following metrics:
...
**The container registry contains an ACR Tasks task named Task1.**

**Task1 is configured to run once every five days**.
...
* ❌ <mark>**A. a notification** → Refers to built-in notifications (email, UI), not webhook integrations</mark>
...
In Azure Repos, you can update work items directly from commit messages using specific keywords.

To **change the state of a work item (like moving it to Resolved)**, you must use action keywords such as:
...
* Link the commit to work item **123**
* Automatically transition its state to **Resolved**

 ❌ Why the other options are wrong:

* **A. #123 fixed** → Incorrect syntax (keyword must come before ID)
* **B. #123 Resolved-** → Invalid format
* **C. Verifies #123-** → Only links, does NOT change state
...
Azure Boards has a work item that has the number 715.
...
✑ <mark>The build must access an on-premises dependency management system.</mark>
...
To answer, drag the appropriate version control systems to the correct projects. 
...
* Requirement:  Minimum **2 reviewers** / Must be linked to **work items**
* ✅ **Git** supports **branch policies**: Required reviewers /Work item linking
...
The application will be deployed to several Azure virtual machines that run Windows Server 2019.
...
You need to create a Dockerfile file to build the image. 

The solution must ensure that the size of the image is minimized.
...
To answer, drag the appropriate policy types to the correct requirements. Each policy type may be used once, more than once, or not at all. You may need to drag the split bar between panes or scroll to view content. NOTE: Each correct selection is worth one point.
...
You use a <mark>**Git repository for applications source control**</mark>.
...
A. progressive exposure （**Canary deployment**)
...
<mark>You need to validate the performance of App1 **in the Staging environment before it is deployed to Production.**</mark>
...
<mark>You need to ensure that you can deploy the update to App1 before App2 becomes available</mark>. You must be able to enable the service in App1 once App2 is deployed.

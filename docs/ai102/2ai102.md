...
1. **Bot asks (Text):** *What is your name?*
    ...
    * On the right side, the *Value* field is set to:

```fx
=coalesce(user.Name, @personName)
```
...
* **Adaptive Card** → supports custom layouts, flexible design (tables, columns, multiple fields).
* **Hero Card** → usually contains a single large image, title, subtitle, text, and buttons.
...
Do you want me to also explain **how this Adaptive Card JSON schema** would look for this itinerary?
...
Got it 👍 Let’s carefully walk through the **.chat file completion** for the Bot Framework Emulator.
...
Would you like me to also **write out the final completed .chat file JSON** so you could test it directly in the Bot Framework Emulator?
...
* **Action dialog** → single action, not step-by-step.
* **Adaptive dialog** → more flexible, event-driven, handles interruptions, but not necessarily linear setup guidance.

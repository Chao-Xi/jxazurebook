...
<span style="color:red">**File size must be less than 50 MB.**</span>
...
* <mark>**Azure Storage** → only required for active learning, but not automatically created in all cases.</mark>
...
| ------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| `api.cognitivemicrosofttranslator.com` + `/translator/text/v3.0/translate?to=en` or `to=fr` | <mark>These endpoints perform **translation**, not detection.</mark>    |
...
**<mark>Custom template: 	Fixed layout forms (e.g., same fields always in same place)</mark>**

**<mark>Custom neural	Unstructured or semi-structured forms with variable layouts</mark>**

---

💡 Explanation

You have **1,000 scanned images of handwritten survey responses** that **do not have a consistent layout**.
...
✅ **Correct choice:** **An Azure Storage account**
...
| **Custom neural**          | Unstructured or semi-structured forms with variable layouts | ✅ Ideal for handwritten, inconsistent forms.          |
...
**Summary:**
Because your forms are **handwritten and not consistently structured**, use a **Custom neural model** trained on documents stored in **Azure Storage** to minimize development effort.
...
You have a **Computer Vision** resource named **contoso1** hosted in the **West US** region.
You want to **generate a resized product image using the smart cropping feature**.

That means you are calling the **Generate Thumbnail** API from the **Computer Vision v3.1** endpoint.

---

✅ Step 1: Correct base URL
...
✅ Step 3: Add smart cropping parameters
...
| Field    | Value                                                                                            |
| -------- | ------------------------------------------------------------------------------------------------ |
| Base URL | **[https://contoso1.cognitiveservices.azure.com](https://contoso1.cognitiveservices.azure.com)** |
| API Path | **/vision/v3.1/generateThumbnail**                                                               |

---

✅ Complete cURL command (for clarity)
...
🧩 1. The historical endpoint format (OLD)
...
🆕 2. The modern (current) endpoint format
...
* The question explicitly says your Computer Vision resource is **named `contoso1`**.
* So you must use the **resource endpoint**, not the **regional one**.
...
| --------------------- | ---------------------------------------------- | ------------------------------ |
| Old regional endpoint | `https://westus.api.cognitive.microsoft.com`   | ❌ Deprecated for new resources |
| New resource endpoint | `https://contoso1.cognitiveservices.azure.com` | ✅ Recommended and required     |
...
![Alt Image Text](../images/ai102_2_48_0.png "Body image")
...
* <mark>This is done by creating a **person model**, uploading sample images of individuals, and associating that model with your videos.</mark>T
...
* <span style="color:red">**Custom Vision** requires training a model and isn’t necessary for standard categories like photograph, drawing, or clipart.</span>
...
* <span style="color:red">**C. Form Recognizer** is used for **structured forms, invoices, receipts — not general magazine text**.</span>
...
* **<mark>B. Enable active learning** → Improves the knowledge base over time by suggesting new question variations, but not for multi-turn</mark>.
...
✅ **Correct Answer: C. a .wav or .mp3 file of the voice talent consenting to the creation of a synthetic version of their voice**

---

💡 Explanation

When you create a **custom neural voice** in **Azure AI Speech**, one of the **required first steps** is to create a **voice talent profile**.
This profile ensures that **Microsoft verifies consent** from the voice talent before training the synthetic voice model.

---

🔍 Step-by-step reasoning

1. **Purpose of the voice talent profile**
...
* **Computer Vision** ✅  or **Azure AI Document Intelligence** ✅
...
- ✅  **Solution: You create a new entity for the domain**.   Yes 
...
- <mark>**Azure AI Vision** is for images, not PDF text extraction</mark>.  
...
- It is designed to detect **multiple people in a space** and can easily tell if more than one person is present. ✅
...
* `"modelId": "prebuilt-healthInsuranceCard.us"` → this means a **prebuilt health insurance card model** was used.
* `"content": "Blood Pressure 118/72"` → this is **medical measurement data**, **not** health insurance information.
* `"confidence"` for text elements:
  ...
  * “118/72”: 0.761
    → all **> 0.70 (70%)**.
* `"fields": {}` → empty, meaning **no form fields were extracted** (so no form elements were recognized).
...
| **The form elements were recognized with greater than 70 percent confidence.** | ❌ No — the `"fields"` object is empty, so no form elements were extracted or recognized.                                                                                        | **No**  |

---

**Final Answers:**

* The chosen model is suitable for the intended use case → **No**
* The text content was recognized with greater than 70 percent confidence → **Yes**
...
<mark>**Prebuilt-read is correct which can classify the test extracted as handwrittern or printed**</mark>
...
You need to add a custom skill to App1 to ensure that the **app can recognize and retrieve properties from <mark>invoices</mark> by using Search1**.
...
![Alt Image Text](../images/ai102_2_148_0.png "Body image")
...
* <mark>**Custom neural models** are best for extracting data when document layouts are variable/unstructured.</mark>
...
2. **Upload sample documents** <mark>(minimum **five** for training; 50 is recommended for production quality, but the question says "minimize effort").</mark>
...
3. **“The chatbot attempts to take the first non-null entity value for userName or personName and assigns the value to user.name.”**  💩
	* ✅ **Yes**.
    * <mark>**The expression `coalesce(user.Name, @personName)` ensures that if `user.Name` already has a value, it’s used; otherwise, it takes the `@personName` entity**.</mark>
...
![Alt Image Text](../images/ai102_2_162_0.png "Body image")
...
![Alt Image Text](../images/ai102_2_163_0.png "Body image")
...
You’re building a chatbot with **Microsoft Bot Framework Composer**.
The goal:
...
* Entities are used for **data extraction** (e.g., recognizing names, dates, or products) from user input — **not** for presenting options.
  ✅ *Used for understanding*, ❌ *not for displaying UI*.
  → **Not applicable**

**B. an Azure function**

* Azure Functions are used to run **custom logic or backend code**, not for displaying UI elements or cards in a chatbot.
  → **Not applicable**

**C. an utterance**

* Utterances are **sample phrases** users might say to trigger intents.
  They are not used for displaying lists or images.
  → **Not applicable**

**D. an adaptive card**

* ✅ Adaptive Cards are used to **display rich content** such as:
...
* ✅ Dialogs are the **core building blocks** of conversations in Bot Framework Composer.
  You use a dialog to manage the flow — e.g., to **show adaptive cards** or handle user responses.
  → **Correct**
...
E. A dialog: Dialogs are used to manage and organize the conversation flow in your chatbot. Y**ou can create a dialog that is responsible for presenting the list of available options using an adaptive card. The dialog can be triggered when needed, and it can handle the user's selection of an option.**
...
3. Deploy the solution to a **new web app**  （Deployment is the last step)
...
1. In the `messages` array:
  ...
  * <mark>(Other roles like `"assistant"` and `"system"` are valid but here the correct one for sending a prompt is `"user"`.)</mark>
...
- **AzureExtensionsOptions**: Specifies Azure extensions, enabling integration with external data sources.
	- **AzureCognitiveSearchChatExtensionConfiguration**: <mark>Configures the Azure Cognitive Search extension, providing the search endpoint, credentials, and index name to access company data</mark>.
...
![Alt Image Text](../images/ai102_2_213_1.png "Body image")
...
![Alt Image Text](../images/ai102_2_214_0.png "Body image")
...
Custom analyzers define **tokenization and normalization** behavior (e.g., handling punctuation or casing),
but not **semantic equivalence** between words.

---

**D. a built-in key phrase extraction skill** ❌

Used in **AI enrichment pipelines** to extract **important terms or topics** from documents,
not for matching user queries to synonyms.

---

✅ **Final Answer:** **A. synonym map**
...
⚠️ While LUIS is used for intent recognition,
you already have QnA Maker — the question implies you need something that **connects** or **routes between them**, not replaces or adds another domain model.

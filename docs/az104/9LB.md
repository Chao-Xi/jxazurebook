...
Backend compute set of virtual machines that o**ffers up the same web server and website.** load balancer solution to balance the traffic for that single web solution.
...
8. Load Balancer Rules:
   - Load balancing rules define how incoming traffic is distributed to the backend pool. They specify the frontend IP configuration, protocol, port, backend port, and backend pool to which the traffic will be distributed. Knowledge of creating and configuring load balancing rules is crucial.
...
10. Traffic Distribution Algorithms:
   - Load balancers distribute traffic using various algorithms such as round-robin, least connections, or least response time. Understanding these algorithms and how they affect traffic distribution is important for the AZ-104 exam.

11. ARM Templates and PowerShell Commands:
   - Ability to deploy and configure Azure Load Balancer using Azure Resource Manager (ARM) templates and PowerShell commands is crucial. Candidates should be familiar with the scripting and automation aspects of load balancer deployment and management.
...
10. URL Path-Based Routing:
   - With URL path-based routing, Application Gateway can direct traffic to different backend pools based on the URL paths requested by clients. This feature is indispensable for multi-tier applications or microservices architectures with distinct endpoints.
...
13. Integration with Azure Monitor:
   - Application Gateway integrates with Azure Monitor, providing insights into its performance, health, and usage. It enables administrators to monitor and gain visibility into the gateway's behavior and make informed decisions based on the collected metrics and logs.
